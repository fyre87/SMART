...
class Earth(BaseEstimator, RegressorMixin, TransformerMixin, MultiOutputMixin):
    ...
    def fit_tree(self, Node, X, y, sample_weight, output_weight, missing):
        ...
        # Remove the forward passer so it can be copied
    ...
    def forward_pass(self, X, y=None,
        ...
        if self.max_depth != None and not isinstance(self.max_depth, int):
            raise ValueError("max_depth must be an integer. Got: " + str(self.max_depth) + " of type: " + str(type(self.max_depth)))
        if self.points_to_validate != None and not isinstance(self.points_to_validate, int) and self.points_to_validate < 0:
            raise ValueError("points_to_validate must be a positive integer. Got: " + str(self.points_to_validate) + " of type: " + str(type(self.points_to_validate)))
        self.Node.max_depth = self.max_depth
        self.Node.points_to_validate = self.points_to_validate
        forward_passer.set_node(self.Node)
        self.Node.set_forward_passer(forward_passer)
        self.Node.run_forward_pass(self.allow_subset, self.subset_method, self.vars_to_consider)
...
#             sample_weight = np.repeat(sample_weight,y.shape[1],axis=1)
    ...
    def get_array_of_leafs(self, curr_node):
        listy = []
        if curr_node.left_child == None and curr_node.right_child == None:
            return [curr_node]
        else:
            # Still have children to traverse
            listy = listy + self.get_array_of_leafs(curr_node.left_child) + self.get_array_of_leafs(curr_node.right_child)
        return listy

    def pruning_pass_leafs(self):
        leaf_array = self.get_array_of_leafs(self.Node)
        # print(leaf_array)

        for i in range(0, len(leaf_array)):
            node = leaf_array[i]
            args = self._pull_pruning_args(**self.__dict__)
            #print(node.forward_passer)
            pruner = PruningPasser(node.Earth.basis_, node.X, node.missing, node.y, node.sample_weight, **args)
            # Run standard MARS pruner on each leaf node
            # print(i)
            pruner.run()
...
#         self.pruning_pass_record_ = record
    ...
    def summary_help(self, Node, current_subset):
        ...
        if Node.left_child == None and Node.right_child == None:
            ...
            print("Training n: ", Node.n)
            print(Node.Earth.summary_individual(), "\n")
            return

        self.summary_help(Node.left_child, current_subset + "\nVar: \"" + str(self.xlabels_[Node.split_var]) +  "\" Less than: " + str(Node.split_val))
        self.summary_help(Node.right_child, current_subset + "\nVar: \"" + str(self.xlabels_[Node.split_var]) + "\" More than: " + str(Node.split_val))
...
#         self.grsq_ = ((1 - (gcv_ / gcv0)) * output_weight).sum()
    ...
    def predict_help(self, Node, X):
        ...
        # This is slow but not the slowest part of the algorithm
        if Node.left_child == None and Node.right_child == None:
